...
export const parse = (
  ...
  return Either.all({
    ...
    Either.map(({ days, hours, minutes, months, seconds, tz, weekdays }) =>
      make({
        tz,
        ...
        and:
          (days.wildcard || weekdays.wildcard) &&
          days.values.size !== 0 &&
          weekdays.values.size !== 0
      })
    )
  )
}
...
export const match = (cron: Cron, date: DateTime.DateTime.Input): boolean => {
  ...
  if (cron.and) {
    return (
      (cron.days.size === 0 || cron.days.has(parts.day)) &&
  ...
  const result = dateTime.mutate(zoned, (current) => {
    ...
    for (let i = 0; i < 10_000; i++) {
      ...
      if (cron.weekdays.size !== 0 || cron.days.size !== 0) {
        if (cron.and) {
          const matchesDay =
            cron.days.size === 0 || cron.days.has(current.getUTCDate())
          const matchesWeekday =
            cron.weekdays.size === 0 || cron.weekdays.has(current.getUTCDay())
...
const parseSegment = (
  ...
  const add =
    wildcard && (first[1] === undefined || first[1] === 1)
      ? constVoid
      : (value: number) => {
  ...
  for (let index = 0; index < fields.length; index++) {
    ...
    if (raw === "*") {
      ...
      if (right === undefined) {
        for (
          let i = left;
          i <= (step === undefined ? left : options.max);
          i += step ?? 1
        ) {
          add(i)
        }
...
const splitStep = (input: string): [string, number | undefined] => {
  ...
  if (seperator !== -1) {
    ...
    return [
      ...
      decimalRegex.test(step) ? Number(step) : NaN
    ]
  }
...
function aliasOrValue(field: string, aliases?: Record<string, number>): number {
  return (
    aliases?.[field.toLocaleLowerCase()] ??
    (decimalRegex.test(field) ? Number(field) : NaN)
  )
}
