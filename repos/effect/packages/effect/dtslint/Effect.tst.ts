...
declare const numberRecord: Record<
  string,
  Effect.Effect<number, "err-4", "dep-4">
>
...
declare const predicateNumbersOrStringsEffect: (
  input: number | string
...
// Tacit helpers
const tacitString = (s: string): Effect.Effect<string> =>
  Effect.succeed(`string ${s}`)
const tacitStringCause = (s: Cause<string>): Effect.Effect<string> =>
  Effect.succeed(`string ${s}`)
const tacitStringPredicate = (_s: string): boolean => true
const tacitStringError = (_s: string): "a" => "a"
const tacitStringErrorEffect = (_s: string): Effect.Effect<never, "a"> =>
  Effect.fail("a")
...
describe("Effect", () => {
  describe("forEach", () => {
    ...
    it("non empty array", () => {
      expect(
        Effect.forEach(readonlyNonEmptyStrings, (a, i) => {
          expect(a).type.toBe<string>()
          expect(i).type.toBe<number>()
          return string
        })
  ...
  describe("all", () => {
    it("tuple", () => {
      ...
      ).type.toBe<Effect.Effect<void, "err-1" | "err-2", "dep-1" | "dep-2">>()
      expect(Effect.all([string, number], { mode: "validate" })).type.toBe<
        Effect.Effect<
          [string, number],
          [Option.Option<"err-1">, Option.Option<"err-2">],
          "dep-1" | "dep-2"
        >
      >()
      expect(
      ...
      expect(Effect.all([string, number], { mode: "either" })).type.toBe<
        Effect.Effect<
          [Either.Either<string, "err-1">, Either.Either<number, "err-2">],
          never,
          "dep-1" | "dep-2"
        >
      >()
      ...
      ).type.toBe<Effect.Effect<void, never, "dep-1" | "dep-2">>()
    })
    ...
    it("struct", () => {
      ...
      expect(
        ...
        Effect.Effect<
          {
            a: Either.Either<string, "err-1">
            b: Either.Either<number, "err-2">
          },
          never,
          "dep-1" | "dep-2"
        >
    ...
    it("array", () => {
      ...
      ).type.toBe<Effect.Effect<void, "err-3", "dep-3">>()
      ...
      ).type.toBe<Effect.Effect<void, Array<Option.Option<"err-3">>, "dep-3">>()
      ...
      ).type.toBe<Effect.Effect<void, never, "dep-3">>()
    })
    ...
    it("record", () => {
      ...
      ).type.toBe<Effect.Effect<void, "err-4", "dep-4">>()
      expect(Effect.all(numberRecord, { mode: "validate" })).type.toBe<
        Effect.Effect<
          { [x: string]: number },
          { [x: string]: Option.Option<"err-4"> },
          "dep-4"
        >
      >()
      expect(
      ...
      expect(Effect.all(numberRecord, { mode: "either" })).type.toBe<
        Effect.Effect<
          { [x: string]: Either.Either<number, "err-4"> },
          never,
          "dep-4"
        >
      >()
      ...
      ).type.toBe<Effect.Effect<void, never, "dep-4">>()
    })
  })
  ...
  describe("allWith", () => {
    it("tuple", () => {
      ...
      expect(
        pipe(
          [string, number] as const,
          Effect.allWith({ discard: true, concurrency: "unbounded" })
        )
      ).type.toBe<Effect.Effect<void, "err-1" | "err-2", "dep-1" | "dep-2">>()
      expect(
      ...
      expect(
        pipe(
          [string, number] as const,
          Effect.allWith({ mode: "validate", discard: true })
        )
      ).type.toBe<
        Effect.Effect<
          void,
      ...
      expect(
        pipe(
          [string, number] as const,
          Effect.allWith({ mode: "either", discard: true })
        )
      ).type.toBe<Effect.Effect<void, never, "dep-1" | "dep-2">>()
    })
    ...
    it("array", () => {
      ...
      ).type.toBe<Effect.Effect<Array<string>, "err-3", "dep-3">>()
    ...
    it("record", () => {
      ...
      ).type.toBe<Effect.Effect<{ [x: string]: number }, "err-4", "dep-4">>()
  ...
  it("filterOrFail", () => {
    expect(
      Effect.succeed("a" as const).pipe(
        Effect.filterOrFail(tacitStringPredicate, (x) => {
  ...
  it("filterOrDie", () => {
    expect(
      Effect.succeed("a" as const).pipe(
        Effect.filterOrDie(tacitStringPredicate, (x) => {
          expect(x).type.toBe<"a">()
          return "fail"
        })
      )
  ...
  it("filterOrDieMessage", () => {
    expect(
      Effect.succeed("a" as const).pipe(
        Effect.filterOrDieMessage(tacitStringPredicate, "fail")
      )
    ).type.toBe<Effect.Effect<"a">>()
  })
  ...
  it("filterOrElse", () => {
    expect(
      Effect.succeed("a" as const).pipe(
        Effect.filterOrElse(tacitStringPredicate, (x) => {
          expect(x).type.toBe<"a">()
    ...
    expect(
      Effect.succeed(numberArray).pipe(
        Effect.filterOrElse(Arr.isNonEmptyArray, () =>
  ...
  it("tap", () => {
    ...
    expect(
      Effect.succeed("a" as const).pipe(
        Effect.tap(tacitString, { onlyEffect: true })
      )
    ).type.toBe<Effect.Effect<"a">>()

    expect(
      Effect.succeed("a" as const).pipe(
        Effect.tap(tacitString("a"), { onlyEffect: true })
      )
    ).type.toBe<Effect.Effect<"a">>()
  })
  ...
  it("catchIf", () => {
    expect(
      ...
      Effect.Effect<Exit<Error | TestError1, Error | TestError1>, never, never>
    >()
  })
  ...
  it("dropWhile", () => {
    expect(
      Effect.dropWhile(numbersArray, (n) => {
        expect(n).type.toBe<number>()
  ...
  it("dropUntil", () => {
    expect(
      Effect.dropUntil(numbersArray, (n) => {
        expect(n).type.toBe<number>()
        return Effect.succeed(true)
      })
    ).type.toBe<Effect.Effect<Array<number>>>()
  ...
  it("filter", () => {
    ...
    expect(
      pipe(
        numberArray,
        Effect.filter((_n: unknown) => Effect.succeed(true))
      )
    ).type.toBe<Effect.Effect<Array<number>>>()
  })
  ...
  it("findFirst", () => {
    expect(
      Effect.findFirst(numberArray, (n) => {
        expect(n).type.toBe<number>()
        return Effect.succeed(true)
      })
    ...
    expect(
      pipe(
        numberArray,
        Effect.findFirst((_n: unknown) => Effect.succeed(true))
      )
    ).type.toBe<Effect.Effect<Option.Option<number>>>()
  })
  ...
  it("reduceEffect", () => {
    expect(
      Effect.reduceEffect(numberEffectIterable, Effect.succeed(0), (n) => {
        expect(n).type.toBe<number>()
        return 0
      })
    ).type.toBe<Effect.Effect<number>>()
  ...
  it("takeUntil", () => {
    expect(
      Effect.takeUntil(numberArray, (n) => {
        expect(n).type.toBe<number>()
        return Effect.succeed(true)
      })
    ).type.toBe<Effect.Effect<Array<number>>>()
    ...
    expect(
      pipe(
        numberArray,
        Effect.takeUntil((_n: unknown) => Effect.succeed(true))
      )
    ).type.toBe<Effect.Effect<Array<number>>>()
  })
  ...
  it("takeWhile", () => {
    expect(
      Effect.takeWhile(numberArray, (n) => {
        expect(n).type.toBe<number>()
        return Effect.succeed(true)
      })
    ).type.toBe<Effect.Effect<Array<number>>>()
    ...
    expect(
      pipe(
        numberArray,
        Effect.takeWhile((_n: unknown) => Effect.succeed(true))
      )
    ).type.toBe<Effect.Effect<Array<number>>>()
  })
  ...
  it("retryOrElse", () => {
    expect(
      Effect.retryOrElse(string, Schedule.forever, (e) => {
        expect(e).type.toBe<"err-1">()
    ...
    expect(
      string.pipe(
        Effect.retryOrElse(Schedule.forever, (e) => {
          expect(e).type.toBe<"err-1">()
          return Effect.succeed(0)
        })
      )
    ...
    expect(
      Effect.retryOrElse(string, Schedule.forever, (_e: string) =>
        Effect.succeed(0)
      )
    ).type.toBe<Effect.Effect<string | number, never, "dep-1">>()
    expect(
      string.pipe(
        Effect.retryOrElse(Schedule.forever, (_e: string) => Effect.succeed(0))
      )
    ).type.toBe<Effect.Effect<string | number, never, "dep-1">>()
  })
  ...
  it("liftPredicate", () => {
    ...
    expect(
      Effect.liftPredicate(
        hole<Predicate.Refinement<string | number, number>>(),
        (sn) => {
    ...
    expect(
      Effect.liftPredicate(primitiveNumber, predicateNumbersOrStrings, (n) => {
        expect(n).type.toBe<number>()
        return "b" as const
      })
    ).type.toBe<Effect.Effect<number, "b">>()
  ...
  it("mapAccum", () => {
    expect(
      Effect.mapAccum(strings, 0, (s, a, i) => {
    ...
    expect(
      Effect.mapAccum(readonlyNonEmptyStrings, 0, (s, a, i) => {
        expect(s).type.toBe<number>()
        expect(a).type.toBe<string>()
        expect(i).type.toBe<number>()
        return Effect.succeed([s + i, a])
  ...
  it("Tag.Proxy", () => {
    ...
    expect(
      hole<Types.Simplify<Effect.Tag.Proxy<"R", { a: () => 1 }>>>()
    ).type.toBe<{ a: () => Effect.Effect<1, never, "R"> }>()
    expect(
      hole<
        Types.Simplify<
          Effect.Tag.Proxy<"R", { a: (...args: ReadonlyArray<number>) => void }>
        >
      >()
    ).type.toBe<{
      a: (...args: ReadonlyArray<number>) => Effect.Effect<void, never, "R">
    }>()
    expect(
      hole<
        Types.Simplify<
          Effect.Tag.Proxy<"R", { a: (...args: [1] | [2, 3]) => void }>
        >
      >()
    ).type.toBe<{
      a: (...args: Readonly<[1] | [2, 3]>) => Effect.Effect<void, never, "R">
    }>()
    expect(
      hole<
        Types.Simplify<
      ...
      a: (...args: Readonly<[1] | [2, 3]>) => Effect.Effect<1, 2, 3 | "R">
    }>()
    expect(hole<Types.Simplify<Effect.Tag.Proxy<"R", { a: 1 }>>>()).type.toBe<{
      a: Effect.Effect<1, never, "R">
    }>()
    expect(
      hole<Types.Simplify<Effect.Tag.Proxy<"R", { a: () => Promise<1> }>>>()
    ).type.toBe<{ a: () => Effect.Effect<1, UnknownException, "R"> }>()
  })

  it("transposeOption", () => {
    expect(Effect.transposeOption(Option.none())).type.toBe<
  ...
  it("transposeMapOption", () => {
    expect(
      Effect.transposeMapOption(Option.none(), (value) => {
        expect(value).type.toBe<never>()
        return string
      })
    ).type.toBe<Effect.Effect<Option.Option<string>, "err-1", "dep-1">>()
    expect(
      pipe(
        Option.none(),
    ...
    expect(
      Effect.transposeMapOption(Option.some(42), (value) => {
  ...
  it("fn", () => {
    ...
    expect(fn).type.toBe<
      (a?: string | undefined) => Effect.Effect<void, never, never>
    >()
  })

  it("fn returns Effect subtype", () => {
    const fnNonGen = Effect.fn(
      (a?: string) => Effect.succeed(a),
      () => Option.some("test")
    )
    const fnGen = Effect.fn(
      function* (a?: string) {
  ...
  it("ensureSuccessType", () => {
    expect(
      Effect.succeed(123).pipe(Effect.ensureSuccessType<number>())
    ).type.toBe<Effect.Effect<number, never, never>>()
  })

  it("ensureErrorType", () => {
    const withoutError = Effect.succeed("no error")
    expect(withoutError.pipe(Effect.ensureErrorType<never>())).type.toBe<
      Effect.Effect<string, never, never>
    >()

    const withError = Effect.fail(new TestError1())
    expect(withError.pipe(Effect.ensureErrorType<TestError1>())).type.toBe<
      Effect.Effect<never, TestError1, never>
    >()
  })
  ...
  it("ensureRequirementsType", () => {
    ...
    expect(
      withoutRequirements.pipe(Effect.ensureRequirementsType<never>())
    ).type.toBe<Effect.Effect<never, never, never>>()

    const withRequirement = Effect.flatMap(TestService, () => Effect.never)
    expect(
      withRequirement.pipe(Effect.ensureRequirementsType<TestService>())
    ).type.toBe<Effect.Effect<never, never, TestService>>()
  })
})
