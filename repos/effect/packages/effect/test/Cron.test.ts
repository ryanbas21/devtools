...
describe("Cron", () => {
  it("parse", () => {
    ...
    deepStrictEqual(
      invalidMinuteTail.left.message,
    ...
    deepStrictEqual(
      invalidDayTail.left.message,
      "Expected a value between 1 and 31"
    )
  ...
  it("next", () => {
    ...
    deepStrictEqual(
      next("23 0-20/2 * * 0", after),
      new Date("2024-01-07 00:23:00")
    )
    deepStrictEqual(next("5 4 * * SUN", after), new Date("2024-01-07 04:05:00"))
    deepStrictEqual(
      next("5 4 * DEC SUN", after),
      new Date("2024-12-01 04:05:00")
    )
    deepStrictEqual(
      next("30 5 0 8 2 *", after),
      new Date("2024-02-08 00:05:30")
    )
    ...
    deepStrictEqual(
      next(Cron.unsafeParse("5 0 8 2 *", london), after),
      DateTime.toDateUtc(londonTime)
    )
    deepStrictEqual(
      next(Cron.unsafeParse("5 0 8 2 *", london), after),
      DateTime.toDateUtc(amsterdamTime)
    )
  })
  ...
  it("next adheres to day-of-month and weekday semantics", () => {
    ...
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 */2 * MON", utc), after),
      new Date("2024-01-15T00:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 1-31/2 * MON", utc), after),
      new Date("2024-01-03T00:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 *,5 * MON", utc), after),
      new Date("2024-01-08T00:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 5,* * MON", utc), after),
  ...
  it("prev", () => {
    ...
    deepStrictEqual(
      prev(Cron.unsafeParse("5 0 8 2 *", utc), before),
      new Date("2023-02-08T00:05:00.000Z")
    )
    deepStrictEqual(
      prev(Cron.unsafeParse("15 14 1 * *", utc), before),
      new Date("2024-01-01T14:15:00.000Z")
    )
    deepStrictEqual(
      prev(Cron.unsafeParse("23 0-20/2 * * * 0", utc), before),
      new Date("2023-12-31T23:20:23.000Z")
    )
    deepStrictEqual(
      prev(Cron.unsafeParse("5 4 * * SUN", utc), before),
      new Date("2023-12-31T04:05:00.000Z")
    )
    deepStrictEqual(
      prev(Cron.unsafeParse("5 4 * DEC SUN", utc), before),
      new Date("2023-12-31T04:05:00.000Z")
    )
    deepStrictEqual(
      prev(Cron.unsafeParse("30 5 0 8 2 *", utc), before),
      new Date("2023-02-08T00:05:30.000Z")
    )

    const wednesday = new Date("2025-10-22T01:00:00.000Z")
    deepStrictEqual(
      prev(Cron.unsafeParse("0 1 * * MON", utc), wednesday),
      new Date("2025-10-20T01:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 1 * * MON", utc), wednesday),
      new Date("2025-10-27T01:00:00.000Z")
    )
    deepStrictEqual(
      prev(Cron.unsafeParse("0 1 * * TUE", utc), wednesday),
      new Date("2025-10-21T01:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 1 * * TUE", utc), wednesday),
      new Date("2025-10-28T01:00:00.000Z")
    )
  })
  ...
  it("forward and reverse sequences stay aligned", () => {
    ...
    for (const [expr, lowerStr, upperStr] of cases) {
      ...
      const forward = gather(
        Cron.sequence(cron, lower),
        lower,
        upper,
        "forward"
      )
      const reverse = gather(
        Cron.sequenceReverse(cron, upper),
        lower,
        upper,
        "reverse"
      ).reverse()
  ...
  it("prev chooses the later occurrence in DST fall-back", () => {
    const make = (s: string) =>
      DateTime.makeZonedFromString(s).pipe(Option.getOrThrow)
    const tz = "Europe/Berlin"
    const cron = Cron.unsafeParse("0 30 2 * * *", tz)
    const before = make("2024-10-27T03:30:00.000+01:00[Europe/Berlin]")
    const result = DateTime.unsafeMakeZoned(prev(cron, before), {
      timeZone: tz
    })
    deepStrictEqual(
      result.pipe(DateTime.formatIsoZoned),
      "2024-10-27T02:30:00.000+02:00[Europe/Berlin]"
    )
  })
  ...
  it("next respects day-of-month or weekday constraints when entering a restricted month", () => {
    ...
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 29 2 MON", tz), after),
      new Date("2024-02-05T00:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 30 2 MON", tz), after),
      new Date("2024-02-05T00:00:00.000Z")
    )
    deepStrictEqual(
      next(Cron.unsafeParse("0 0 31 2 MON", tz), after),
      new Date("2024-02-05T00:00:00.000Z")
    )
  })
  ...
  it("sequenceReverse", () => {
    ...
    const utc = DateTime.zoneUnsafeMakeNamed("UTC")
    const generator = Cron.sequenceReverse(
      Cron.unsafeParse("23 0-20/2 * * 0", utc),
      start
    )
  ...
  it("handles leap years", () => {
    ...
    deepStrictEqual(
      next("0 0 29 2 *", new Date("2024-01-01 00:00:00")),
      new Date("2024-02-29 00:00:00")
    )
    deepStrictEqual(
      next("0 0 29 2 *", new Date("2025-01-01 00:00:00")),
      new Date("2028-02-29 00:00:00")
    )
  })

  it("handles transition into daylight savings time", () => {
    const make = (date: string) =>
      DateTime.makeZonedFromString(date).pipe(Option.getOrThrow)
    ...
    deepStrictEqual(
      next().pipe(DateTime.formatIsoZoned),
  ...
  it("handles transition out of daylight savings time", () => {
    ...
    deepStrictEqual(
      next().pipe(DateTime.formatIsoZoned),
      e.pipe(DateTime.formatIsoZoned)
    )
  })
  ...
  it("handles utc timezone", () => {
    ...
    const make = (date: string) =>
      DateTime.makeZonedFromString(date).pipe(Option.getOrThrow)
    const sequence = Cron.sequence(
      Cron.unsafeParse("30 * * * *", utc),
      make("2024-10-27T00:00:00.000+00:00[UTC]")
    )
    const next = (): DateTime.Zoned =>
      DateTime.unsafeMakeZoned(sequence.next().value, { timeZone: utc })
