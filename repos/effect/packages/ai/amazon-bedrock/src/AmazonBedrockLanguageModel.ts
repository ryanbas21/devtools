...
export declare namespace Config {
  ...
  export interface Service extends Simplify<
    Partial<
      Omit<typeof ConverseRequest.Encoded, "messages" | "system" | "toolConfig">
    >
  > {}
}
...
export const make = Effect.fnUntraced(function* (options: {
  ...
  const makeRequest = Effect.fnUntraced(function* (
    ...
    const { additionalTools, betas, toolConfig } = yield* prepareTools(
      providerOptions,
      config
    )
    ...
    const request: typeof ConverseRequest.Encoded = {
      ...
      ...(responseFormat.type === "json"
              ...
              tools: [
                {
                  toolSpec: {
                    ...
                    description:
                      Tool.getDescriptionFromSchemaAst(
                        responseFormat.schema.ast
                      ) ?? "Respond with a JSON object",
                    inputSchema: {
                      json: Tool.getJsonSchemaFromSchemaAst(
                        responseFormat.schema.ast
                      ) as any
                    }
                  }
        ...
        : Predicate.isNotUndefined(toolConfig.tools) &&
  ...
  return yield* LanguageModel.make({
    ...
    streamText: Effect.fnUntraced(
      ...
      (effect, options) =>
        effect.pipe(
          Effect.flatMap(({ request, stream }) =>
            makeStreamResponse(request, stream, options)
          ),
          Stream.unwrap,
          Stream.map((response) => {
                  ...
                  case "file": {
                    if (part.data instanceof URL) {
                      ...
                      return yield* new AiError.MalformedInput({
                        ...
                        method: "prepareMessages",
                        description:
                          "File URL inputs are not supported at this time"
                      })
                    }
...
const makeStreamResponse: (
  ...
  never,
  IdGenerator.IdGenerator
> = Effect.fnUntraced(function* (request, stream, options) {
          ...
          case "contentBlockDelta": {
            ...
            switch (delta.type) {
              ...
              case "toolUse": {
                const block = contentBlocks[index]
                if (
                  Predicate.isNotUndefined(block) &&
...
> = Effect.fnUntraced(function* (options, config) {
  ...
  if (
    !hasAnthropicTools &&
    tools.length > 0 &&
    Predicate.isNotUndefined(options.toolChoice)
  ) {
    if (options.toolChoice === "none") {
      ...
      toolChoice =
        options.toolChoice.mode === "auto" ? { auto: {} } : { any: {} }
    }
  }

  const toolConfig: Partial<typeof ToolConfiguration.Encoded> =
...
const getCachePoint = (
  ...
  part.options.bedrock?.cachePoint
...
const getDocumentFormat: (
  mediaType: string
) => Effect.Effect<typeof DocumentFormat.Encoded, AiError.AiError> =
  Effect.fnUntraced(function* (mediaType) {
    ...
    if (Predicate.isUndefined(format)) {
      return yield* new AiError.MalformedInput({
        ...
        description:
          `Unsupported document MIME type: ${mediaType} - expected ` +
...
const getImageFormat: (
  mediaType: string
) => Effect.Effect<typeof ImageFormat.Encoded, AiError.AiError> =
  Effect.fnUntraced(function* (mediaType) {
    const format = ImageFormat.literals.find(
      (format) => mediaType === `image/${format}`
    )
    ...
    if (Predicate.isUndefined(format)) {
      return yield* new AiError.MalformedInput({
        ...
        description:
          `Unsupported image MIME type: ${mediaType} - expected ` +
