...
describe("AmazonBedrockLanguageModel", () => {
  describe("makeRequest / additionalModelRequestFields", () => {
    it.effect(
      "strips thinking when using generateObject (forced toolChoice.tool)",
      () =>
        Effect.gen(function* () {
          ...
          yield* LanguageModel.generateObject({
            ...
            Effect.provide(
              makeCapturingLayer(captured, toolUseResponse(TestObjectName))
            )
          )
          ...
          assert.deepStrictEqual(req.toolConfig?.toolChoice, {
            tool: { name: TestObjectName }
          })
    ...
    it.effect(
      "preserves thinking when using generateText (no forced toolChoice)",
      () =>
        Effect.gen(function* () {
          const captured: Array<typeof ConverseRequest.Encoded> = []
          ...
          assert.deepStrictEqual(
            req.additionalModelRequestFields?.["thinking"],
            {
              type: "enabled",
              budget_tokens: 5000
            }
    ...
    it.effect(
      "does not set additionalModelRequestFields when none configured and using generateObject",
      () =>
        Effect.gen(function* () {
          const captured: Array<typeof ConverseRequest.Encoded> = []
    ...
    it.effect(
      "preserves non-thinking fields in additionalModelRequestFields with generateObject",
      () =>
        Effect.gen(function* () {
          const captured: Array<typeof ConverseRequest.Encoded> = []
          ...
          assert.isUndefined(req.additionalModelRequestFields?.["thinking"])
          assert.strictEqual(
            req.additionalModelRequestFields?.["someOtherField"],
            "preserved"
          )
        })
    )
