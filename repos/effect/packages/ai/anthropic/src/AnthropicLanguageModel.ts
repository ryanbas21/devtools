...
export class Config extends Context.Tag(
  ...
  static readonly getOrUndefined: Effect.Effect<
    typeof Config.Service | undefined
...
export declare namespace Config {
   ...
   */
  export interface Service extends Simplify<
    Partial<
      Omit<
    ...
    readonly anthropic?:
      | {
          ...
          readonly cacheControl?:
            | typeof Generated.CacheControlEphemeral.Encoded
            | undefined
        }
      | undefined
  }
  ...
  export interface FilePartOptions extends ProviderOptions {
    readonly anthropic?:
      | {
           ...
           */
          readonly citations?:
            | typeof Generated.RequestCitationsConfig.Encoded
            | undefined
          /**
          ...
          : [
              {
                ...
                description:
                  Tool.getDescriptionFromSchemaAst(responseFormat.schema.ast) ??
                  "Respond with a JSON object",
                input_schema: Tool.getJsonSchemaFromSchemaAst(
                  responseFormat.schema.ast
                ) as any
              }
            ],
      tool_choice:
...
const prepareMessages: (
...
) => Effect.Effect<
  {
    ...
    readonly system:
      | ReadonlyArray<typeof Generated.BetaRequestTextBlock.Encoded>
      | undefined
...
> = Effect.fnUntraced(function* (options) {
  ...
  let system: Array<typeof Generated.BetaRequestTextBlock.Encoded> | undefined =
    undefined
...
> = Effect.fnUntraced(function* (response, options) {
    ...
    switch (part.type) {
        ...
        if (options.responseFormat.type === "text") {
          ...
          if (Predicate.isNotNullable(part.citations)) {
            for (const citation of part.citations) {
              const source = yield* processCitation(
                citation,
                citableDocuments,
                idGenerator
              )
      ...
      case "thinking": {
        parts.push({
          ...
          text: part.thinking,
          metadata: {
            anthropic: { type: "thinking", signature: part.signature }
          }
        })
        break
      }
      ...
      case "redacted_thinking": {
        parts.push({
          ...
          text: "",
          metadata: {
            anthropic: { type: "redacted_thinking", redactedData: part.data }
          }
        })
        break
      }
      ...
      case "tool_use": {
        ...
        if (options.responseFormat.type === "json") {
          ...
          const providerTool = AnthropicTool.getProviderDefinedToolName(
            part.name
          )
          const name = Predicate.isNotUndefined(providerTool)
            ? providerTool
            : part.name
          const providerName = Predicate.isNotUndefined(providerTool)
            ? part.name
            : undefined
          parts.push({
            type: "tool-call",
...
const makeStreamResponse: (
  ...
  never,
  IdGenerator.IdGenerator
> = Effect.fnUntraced(function* (stream, options) {
  const idGenerator = yield* IdGenerator.IdGenerator
  const citableDocuments = extractCitableDocuments(options.prompt)
      ...
      Effect.fnUntraced(function* (event) {
        ...
        switch (event.type) {
          ...
          case "message_delta": {
            ...
            usage.totalTokens =
              (usage.inputTokens ?? 0) + (event.usage.output_tokens ?? 0)
          ...
          case "content_block_start": {
            ...
            switch (event.content_block.type) {
              ...
              case "tool_use": {
                ...
                const providerTool =
                  AnthropicTool.getProviderDefinedToolName(toolName)
                const name = Predicate.isNotUndefined(providerTool)
                  ? providerTool
                  : toolName
                const providerName = Predicate.isNotUndefined(providerTool)
...
> = Effect.fnUntraced(function* (options, config) {
  ...
  let toolChoice: typeof Generated.BetaToolChoice.Encoded | undefined =
    undefined
...
const isCitationPart = (
  part: Prompt.UserMessage["content"][number]
): part is Prompt.FilePart => {
  if (
    part.type === "file" &&
    (part.mediaType === "application/pdf" || part.mediaType === "text/plain")
  ) {
...
const extractCitableDocuments = (
  prompt: Prompt.Prompt
): ReadonlyArray<CitableDocument> => {
...
const getCacheControl = (
  ...
  part.options.anthropic?.cacheControl
...
const shouldEnableCitations = (part: Prompt.FilePart): boolean =>
  part.options.anthropic?.citations?.enabled ?? false
...
> = Effect.fnUntraced(function* (citation, citableDocuments, idGenerator) {
