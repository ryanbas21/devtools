    ...
    test("two registered APIs use their own error schemas (regression #6243)", async () => {
      const Api1 = HttpApi.make("Api1").add(
        HttpApiGroup.make("group1").add(
          HttpApiEndpoint.get("endpoint1")`/1`.addError(
            Schema.transformLiteral("BAD", "x")
          )
        )
      )
      const Handlers1 = HttpApiBuilder.group(Api1, "group1", (_) =>
        _.handle("endpoint1", () => Effect.fail("x" as const))
      )
      ...
      const Api2 = HttpApi.make("Api2").add(
        HttpApiGroup.make("group2").add(
          HttpApiEndpoint.get("endpoint2")`/2`.addError(
            Schema.transformLiteral("GOOD", "x")
          )
        )
      )
      const Handlers2 = HttpApiBuilder.group(Api2, "group2", (_) =>
        _.handle("endpoint2", () => Effect.fail("x" as const))
      )
      ...
      strictEqual(
        body1,
        '"BAD"',
        "Api1's endpoint must use Api1's error schema"
      )
      ...
      strictEqual(
        body2,
        '"GOOD"',
        "Api2's endpoint must use Api2's error schema, not Api1's"
      )
    })
    ...
    test("single registered API still encodes errors using its own schema", async () => {
      const Api = HttpApi.make("Api").add(
        HttpApiGroup.make("group").add(
          HttpApiEndpoint.get("endpoint")`/`.addError(
            Schema.transformLiteral("OK", "x")
          )
        )
      )
      const Handlers = HttpApiBuilder.group(Api, "group", (_) =>
        _.handle("endpoint", () => Effect.fail("x" as const))
      )
