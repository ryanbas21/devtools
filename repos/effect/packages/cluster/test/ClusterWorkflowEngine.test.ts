...
describe.concurrent("ClusterWorkflowEngine", () => {
    ...
    Effect.gen(function* () {
      ...
      const token = yield* DurableDeferred.token(EmailTrigger).pipe(
        Effect.provideService(
          WorkflowInstance,
          WorkflowInstance.initial(EmailWorkflow, executionId)
        )
      )
      ...
      // test poll
      expect(yield* EmailWorkflow.poll(executionId)).toEqual(
  ...
  it.effect("routes durable clock wakeups to the workflow shard group", () =>
    Effect.gen(function* () {
      ...
      const envelope = driver.journal.find(
        (envelope) =>
  ...
  it.effect(
    "routes durable deferred completions to the workflow shard group after a partial client is cached",
    () =>
      Effect.gen(function* () {
        const driver = yield* MessageStorage.MemoryDriver
        const engine = yield* WorkflowEngine
        const executionIdBeforeRegister =
          yield* ShardedDeferredWorkflow.executionId({ id: "before-register" })
        const tokenBeforeRegister = DurableDeferred.tokenFromExecutionId(
          ShardedDeferred,
          {
        ...
        const executionIdAfterRegister =
          yield* ShardedDeferredWorkflow.executionId({ id: "after-register" })
        const tokenAfterRegister = DurableDeferred.tokenFromExecutionId(
          ShardedDeferred,
          {
            workflow: ShardedDeferredWorkflow,
  ...
  it.effect(
    "forwards parent pointer when spawning a child with discard:true",
    () =>
      Effect.gen(function* () {
        const driver = yield* MessageStorage.MemoryDriver
        const fiber = yield* DiscardParentWorkflow.execute({
          id: "discard-parent-1"
        }).pipe(Effect.fork)
        yield* TestClock.adjust(1)
        yield* Fiber.join(fiber)
        ...
        assert.exists(
          parentRun,
          "expected a run envelope for the parent workflow"
        )
        assert.exists(
          childRun,
          "expected a run envelope for the child workflow"
        )

        const childPayload = (childRun as { payload: Record<string, unknown> })
          .payload
        ...
        expect(parent!.executionId).toEqual(
          (parentRun as { address: { entityId: string } }).address.entityId
        )
      }).pipe(Effect.provide(TestWorkflowLayer))
  )
})
  ...
  Effect.fnUntraced(function* () {
    ...
    return yield* Activity.raceAll("race", [
      Activity.make({
        ...
        success: Schema.String,
        error: Schema.Never,
        execute: Effect.onInterrupt(
          Effect.delay(Effect.succeed("Activity1"), 1000),
          () =>
            Effect.sync(() => {
      ...
      Activity.make({
        ...
        error: Schema.Never,
        execute: Effect.onInterrupt(
          Effect.delay(Effect.succeed("Activity2"), 500),
          () =>
            Effect.sync(() => {
      ...
      Activity.make({
        ...
        error: Schema.Never,
        execute: Effect.onInterrupt(
          Effect.delay(Effect.succeed("Activity3"), 100),
          () =>
            Effect.sync(() => {
...
const ShardedClockWorkflowLayer = ShardedClockWorkflow.toLayer(
  Effect.fnUntraced(function* () {
    yield* DurableClock.sleep({
      name: "ShardedClock",
      duration: "10 seconds",
...
const DiscardParentWorkflowLayer = DiscardParentWorkflow.toLayer(
  Effect.fnUntraced(function* ({ id }) {
    yield* DiscardChildWorkflow.execute(
      { id: `${id}-child` },
      { discard: true }
    )
  })
