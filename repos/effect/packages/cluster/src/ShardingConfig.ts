 ...
 * @category models
 */
export class ShardingConfig extends Context.Tag(
...
const defaultRunnerAddress = RunnerAddress.make({
  host: "localhost",
  port: 34431
})
 ...
 */
export const layer = (
  options?: Partial<ShardingConfig["Type"]>
): Layer.Layer<ShardingConfig> =>
...
export const config: Config.Config<ShardingConfig["Type"]> = Config.all({
  runnerAddress: Config.all({
    ...
    Config.map((options) => RunnerAddress.make(options)),
    Config.option
  ),
  ...
  availableShardGroups: Config.array(
    Config.string("availableShardGroups")
  ).pipe(
  ...
  refreshAssignmentsInterval: Config.duration(
    "refreshAssignmentsInterval"
  ).pipe(
    Config.withDefault(defaults.refreshAssignmentsInterval),
    Config.withDescription(
      "The interval at which to refresh shard assignments."
    )
  ),
  ...
  simulateRemoteSerialization: Config.boolean(
    "simulateRemoteSerialization"
  ).pipe(
    Config.withDefault(defaults.simulateRemoteSerialization),
    Config.withDescription(
      "Simulate serialization and deserialization to remote runners for local entities."
    )
  )
})
...
export const layerFromEnv = (
  ...
  Layer.effect(
    ShardingConfig,
    options
      ? Effect.map(configFromEnv, (config) => ({ ...config, ...options }))
      : configFromEnv
  )
