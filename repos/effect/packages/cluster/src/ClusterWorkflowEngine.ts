...
export const make = Effect.gen(function* () {
  ...
  const clientsPartial = yield* RcMap.make({
    lookup: Effect.fnUntraced(function* (workflowName: string) {
      const entity =
        entities.get(workflowName) ?? ensurePartialEntity(workflowName)
      return yield* entity.client
    }),
    idleTimeToLive: "5 minutes"
  })
  ...
  const replyForRequestId = Effect.fnUntraced(function* (
  ...
  const sendResumeParent = Effect.fnUntraced(function* (options: {
    ...
    if (Option.isNone(requestId)) {
      const client = (yield* RcMap.get(clientsPartial, options.workflowName))(
        options.executionId
      )
  ...
  const engine = makeUnsafe({
            ...
            Effect.gen(function* () {
              ...
              return {
                run: (request: Entity.Request<any>) => {
                  ...
                  return execute(
                    workflow.payloadSchema.make(payload),
                    executionId
                  ).pipe(
                    Effect.onExit((exit) => {
                      const suspendOnFailure = Context.get(
                      ...
                      if (
                        ...
                        return parent
                          ? ensureSuccess(sendResumeParent(parent))
                          : Effect.void
                      }
                      return engine.deferredResult(InterruptSignal).pipe(
                        Effect.flatMap((maybeExit) => {
                          ...
                          return Effect.zipRight(
                            Effect.ignore(
                              clearClock({ workflow, executionId })
                            ),
                ...
                activity(request: Entity.Request<any>) {
                  ...
                  return Effect.gen(function* () {
                    ...
                    Effect.catchAllCause((cause) => {
                      ...
                      // client requested it
                      const ids = Array.from(interruptors, (id) =>
                        Array.from(FiberId.ids(id))
                      ).flat()
                      const suspend = ids.includes(
                        RpcServer.fiberIdClientInterrupt.id
                      )
                      if (suspend) {
                ...
                deferred: Effect.fnUntraced(function* (
    ...
    poll: Effect.fnUntraced(function* (workflow, executionId) {
      ...
      const exit = yield* Schema.decode(exitSchema)(
        oreply.value.exit
      ) as Effect.Effect<Exit.Exit<any, any>, ParseResult.ParseError>
      return yield* exit
    }, Effect.orDie),
        ...
        const nonSuspendedReply = oreply.pipe(
          Option.filter(
            (reply) =>
              ...
              reply.exit.value._tag !== "Suspended"
          )
        )
    ...
    activityExecute: Effect.fnUntraced(function* (activity, attempt) {
      ...
      const client = (yield* RcMap.get(clientsPartial, instance.workflow.name))(
      ...
      while (true) {
        ...
        const result = yield* Effect.orDie(
          client.activity({ name: activity.name, attempt })
        )
        ...
        if (
          ...
          (activities.has(activityId) || interruptedActivities.has(activityId))
        ) {
...
const ensureSuccess = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
  effect.pipe(Effect.sandbox, Effect.retry(retryPolicy), Effect.orDie)
  ...
  Entity.make(`Workflow/${workflow.name}`, [
    Rpc.make("run", {
      payload: {
        ...
        [payloadParentKey]: Schema.optional(
          Schema.Struct({
...
const decodeDeferredWithExit = Schema.decodeSync(
  Reply.WithExit.schema(DeferredRpc)
)
...
const activityPrimaryKey = (activity: string, attempt: number) =>
  `${activity}/${attempt}`

class ClockPayload extends Schema.Class<ClockPayload>(
  `Workflow/DurableClock/Run`
)({
  name: Schema.String,
  workflowName: Schema.String,
